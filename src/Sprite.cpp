...
void Sprite::updateHitBox() {
    //Gets a hitbox using a radius from the average height and width of sprite
    if (!bShowImage) {
        //Adjusts width and height using scale vector
        float w = scale.x * abs(verts[0].x - verts[1].x);
        float h = scale.y * abs(verts[2].y - verts[0].y);
        ...
        float w = spriteImage.getWidth();
        float h = spriteImage.getHeight();
